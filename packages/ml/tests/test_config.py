...
def test_fixed_acoustic_constants_unchanged():
    # Authoritative BUILD_SPEC constants. If a change here is intentional it must
    # be flagged + justified in docs/BUILD_SPEC.md — not silently tuned.
    assert config.SAMPLE_RATE == 16_000
    assert config.BAND_LOW_HZ == 300
    assert config.BAND_HIGH_HZ == 3_000
